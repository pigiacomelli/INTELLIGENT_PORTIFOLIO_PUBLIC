...
export const PortfolioSummary: React.FC<PortfolioSummaryProps> = ({ caixaTotal, totalValue, assetCount, lucroPrejuizo, lucroPrejuizoPercentual, vertical }) => {
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                <div className="glass-card animate-fade-in" style={{ padding: '2rem', background: 'rgba(30, 41, 59, 0.3)', border: '1px solid var(--glass-border)' }}>
                    ...
                    <p style={{ margin: '0', fontSize: 'clamp(1.75rem, 7vw, 2.5rem)', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-1.5px', overflowWrap: 'anywhere' }}>
                        <span style={{ color: 'var(--accent-blue)', marginRight: '6px', fontSize: '1.8rem' }}>R$</span>
                        {(totalValue + caixaTotal).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                    {lucroPrejuizo !== undefined && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '1.2rem', padding: '0.8rem', borderRadius: '12px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--glass-border)' }}>
                            ...
                            <span style={{ fontSize: '1.1rem', fontWeight: 800, color: plColor }}>
                                {isPositive ? '+' : ''}R$ {lucroPrejuizo.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                ...
                {caixaTotal > 0 && (
                    <div className="glass-card animate-fade-in" style={{ padding: '1.4rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(16, 185, 129, 0.03)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                        ...
                        <span style={{ fontSize: '1.3rem', fontWeight: 900, color: 'var(--text-main)' }}>
                            R$ {caixaTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </span>
                    </div>
                )}
    ...
    return (
        <div style={{ display: 'flex', gap: '1.5rem', width: '100%', flexWrap: 'wrap' }} className="portfolio-summary animate-fade-in">
            {caixaTotal > 0 && (
                <div className="glass-card animate-shimmer" style={{ padding: '2rem 2.5rem', background: 'rgba(16, 185, 129, 0.05)', border: '1px solid rgba(16, 185, 129, 0.15)', display: 'flex', flexDirection: 'column', justifyContent: 'center', minWidth: '220px', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <p style={{ margin: '0', fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-1px', position: 'relative', zIndex: 1, overflowWrap: 'anywhere' }}>
                        <span style={{ color: 'var(--accent-emerald)', fontSize: '1.5rem', marginRight: '6px' }}>R$</span>
                        {caixaTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                </div>
            )}
            ...
            <div className="glass-card portfolio-summary__main" style={{ padding: '2rem 3rem', flex: '1 1 520px', minWidth: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(135deg, rgba(30, 41, 59, 0.4) 0%, rgba(15, 23, 42, 0.4) 100%)', border: '1px solid var(--glass-border)' }}>
                <div>
                    ...
                    <p className="portfolio-summary__total" style={{ margin: '0', fontSize: 'clamp(2rem, 4.5vw, 3.5rem)', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-2px', overflowWrap: 'anywhere' }}>
                ...
                {lucroPrejuizo !== undefined && (
                    <div className="portfolio-summary__result" style={{ borderLeft: '1px solid var(--glass-border)', paddingLeft: '3rem', display: 'flex', flexDirection: 'column', justifyContent: 'center', minWidth: '260px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
                            <p style={{ margin: '0', fontSize: 'clamp(1.55rem, 3vw, 2.2rem)', fontWeight: 900, color: plColor, letterSpacing: '-1px', overflowWrap: 'anywhere' }}>
                                {isPositive ? '+' : ''}R$ {lucroPrejuizo.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </p>
